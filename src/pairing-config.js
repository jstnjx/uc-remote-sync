  ...
  return String(value || "")
    ...
    .map((entry, index) => {
      ...
      if (isPairingIdentifier(parts[0])) {
        ...
        throw new Error(`Invalid child entry ${index + 1}; expected IDENTIFIER|TOKEN or NAME|IDENTIFIER|TOKEN`);
      }

      identifier = identifier ? displayPairingIdentifier(identifier) : null;
      if (!token || token.length < 16) throw new Error(`Invalid token in child entry ${index + 1}`);
