...
export function isUsableIpv4Address(value) {
  ...
  if (octets.every((part) => part === 255)) return false;
  return true;
}
...
export function remoteSyncServiceFromHealth(remote, value, agentPort = DEFAULT_AGENT_PORT) {
  ...
  return {
    ...
    discovery: "remote-mdns-health"
  };
}

export class RemoteSyncMdnsPublisher {
  constructor({ identifier, port = DEFAULT_AGENT_PORT, name = "Remote Sync child", version = "unknown", nodeId = null, ready = true, interfaceValue = null }) {
  ...
  packet() {
    ...
    const additionals = [
      ...
      resourceRecord(this.instance, TYPE_TXT, txtPayload({ id: this.identifier, role: "child", name: this.name, ver: this.version, node: this.nodeId || "", ready: this.ready ? "1" : "0", state: this.ready ? "ready" : "paired" }), 120, true),
  ...
  async start() {
    ...
    log.info(`Advertising child ${this.identifier} (${this.ready ? "ready" : "paired"}) on ${advertised}:${this.port}`);
  }
...
export class RemoteSyncDiscovery {
  ...
  async resolve(identifier, { timeoutMs = this.timeoutMs, force = false } = {}) {
    ...
    if (!normalized) throw new Error("Invalid child identifier");
    const cached = this.cache.get(normalized);
    if (!force && cached && cached.expiresAt > Date.now()) return cached.value;
    const values = await this.#discoverAll(timeoutMs);
    const value = values.find((item) => normalizePairingIdentifier(item.identifier) === normalized);
    if (!value) {
      const error = new Error(`Child ${displayPairingIdentifier(identifier)} was not discovered over mDNS`);
  ...
  async #discoverAll(timeoutMs) {
    ...
    log.info(`Discovered ${values.length} Remote Sync child service(s): ${values.map((item) => `${item.identifier}@${item.address || item.hostname}`).join(", ") || "none"}`);
    return values;
  }
  ...
  async #probeRemoteAgents(remoteDevices) {
    ...
    const attempts = remoteDevices.map(async (remote) => {
      ...
      try {
        const response = await this.fetchImpl(`${baseUrl}/health`, { signal: controller.signal });
        if (!response.ok) return null;
        const value = await response.json();
        return remoteSyncServiceFromHealth(remote, value, this.agentPort);
      } catch (error) {
        if (error?.name !== "AbortError") log.debug(`Remote Sync health probe failed for ${baseUrl}:`, error.message);
        return null;
      } finally {
        clearTimeout(timer);
      }
