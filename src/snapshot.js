...
import { MAX_SNAPSHOT_BYTES, SCHEMA_VERSION } from "./constants.js";
import { canonicalJson, firstIdentifier, sha256Bytes, utcNow } from "./util.js";
...
export class SnapshotReader {
  static async read(payload) {
    ...
    if (!manifest || ![4, SCHEMA_VERSION].includes(incomingSchema)) throw new Error(`Unsupported snapshot schema ${manifest?.schema_version}`);
    const resources = {};
