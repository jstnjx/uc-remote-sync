...
import { DEFAULT_VIRTUAL_DOCK_PORT } from "./constants.js";
import { connectWebSocket, createWebSocketHttpServer, WebSocketPeer } from "./websocket.js";
...
export async function bridgeMasterDockTunnel({ downstream, physicalUrl, physicalToken, virtualToken, connect = connectWebSocket }) {
  if (!physicalUrl) throw new Error("The master Core did not expose a physical Dock WebSocket URL");
  if (!physicalToken) throw new Error("No physical Dock API token is configured on the master; reconfigure Remote Sync and enter the Dock token");
...
export class VirtualDockServer {
  ...
  async #connection(peer, request) {
    ...
    try {
      ...
      if (!pairing.master_agent_url || !pairing.master_command_token) throw new Error("Master Dock tunnel is not configured");
      ...
      log.info(`Virtual Dock ${sourceDockId} connected to master tunnel`);
    } catch (error) {
      log.warn(`Virtual Dock ${sourceDockId || "connection"} failed:`, error.message);
      closePeer(peer, 1011, "Master Dock unavailable");
    }
  }
}
